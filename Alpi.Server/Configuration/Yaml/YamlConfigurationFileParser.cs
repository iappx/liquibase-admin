...
namespace Alpi.Server.Configuration.Yaml
{
    internal class YamlConfigurationFileParser
    {
        ...
        private void VisitYamlScalarNode(string context, YamlScalarNode yamlValue)
        {
            ...
            string value = IsNullValue(yamlValue) ? null : yamlValue.Value;
            if (value != null)
            {
                string pattern = @"\$\{[A-Za-z0-9_]+\}";
                Regex rg = new Regex(pattern);
                MatchCollection matches = rg.Matches(value);

                for (int i = 0; i < matches.Count; i++)
                {
                    string envVariableName = matches[i].Value.Substring(2, matches[i].Value.Length - 3);
                    string envValue = Environment.GetEnvironmentVariable(envVariableName);
                    value = value.Replace(matches[i].Value, envValue);
                }
            }

            _data[currentKey] = value;
            ExitContext();
        }
